...
export function EditPollPage() {
  ...
  const existingSuggestions = useMemo(() => {
    ...
    if (matchDay.matchId && canFinalizeAppointment(matchDay)) {
      return [...matchDays]
        .filter((item) => item.matchId === matchDay.matchId && canFinalizeAppointment(item))
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const newSuggestions = normalizeSuggestionDrafts(suggestions);
